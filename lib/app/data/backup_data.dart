import 'package:tap_and_jot/app/models/api_model.dart';

final List<Quotes> backupQuotes = [
  Quotes(
      text: "The only way to do great work is to love what you do.",
      author: "Steve Jobs"),
  Quotes(text: "Leap, and the net will appear.", author: "John Burroughs"),
  Quotes(
      text: "Believe you can and you're halfway there.",
      author: "Theodore Roosevelt"),
  Quotes(
      text: "In the middle of difficulty lies opportunity.",
      author: "Albert Einstein"),
  Quotes(
      text: "The secret of getting ahead is getting started.",
      author: "Mark Twain"),
  Quotes(
      text: "Don't watch the clock; do what it does. Keep going.",
      author: "Sam Levenson"),
  Quotes(
      text:
          "Your present circumstances don't determine where you can go; they merely determine where you start.",
      author: "Nido Qubein"),
  Quotes(
      text:
          "The only limit to our realization of tomorrow will be our doubts of today.",
      author: "Franklin D. Roosevelt"),
  Quotes(
      text: "You miss 100% of the shots you don't take.",
      author: "Wayne Gretzky"),
  Quotes(
      text: "Opportunities don't happen. You create them.",
      author: "Chris Grosser"),
  Quotes(
      text: "Life begins at the end of your comfort zone.",
      author: "Neale Donald Walsch"),
  Quotes(
      text:
          "The road to success and the road to failure are almost exactly the same.",
      author: "Colin R. Davis"),
  Quotes(
      text: "The future rewards those who press on.", author: "Barack Obama"),
  Quotes(
      text: "The best way to predict the future is to create it.",
      author: "Peter Drucker"),
  Quotes(
      text:
          "Success is walking from failure to failure with no loss of enthusiasm.",
      author: "Winston Churchill"),
  Quotes(
      text:
          "You don't have to be great to start, but you have to start to be great.",
      author: "Zig Ziglar"),
  Quotes(
      text: "It does not matter how slowly you go as long as you do not stop.",
      author: "Confucius"),
  Quotes(
      text: "The only impossible journey is the one you never begin.",
      author: "Tony Robbins"),
  Quotes(
      text: "When nothing is sure, everything is possible.",
      author: "Margaret Drabble"),
  Quotes(
      text: "If you want to make God laugh, tell him about your plans.",
      author: "Woody Allen"),
];
